  ...
  return (
    <div className="grid grid-cols-3 gap-4">
      {technologies.map((technology) => (
        <div key={technology.name} className="rounded-[20px] border border-[#e8edf3] bg-white p-[14px] hover:border-[#d91b7e]">
          <div className="flex items-start justify-between">
            <img src={technology.icon} alt={technology.name} className="h-[55px] w-[55px] object-contain"/>
          ...
          <button onClick={() => handleAddToStack(technology)} disabled={added.has(technology.name)}
            className={`mt-5 h-12 w-full rounded-[10px] 
              text-base font-semibold ${added.has(technology.name) ? "bg-white text-black" : "bg-[#080d1b] text-white"
            }`}>
