    ...
    <div className="container mx-auto p-4 px-25">
      ...
      <div className="mt-8 flex gap-8">
        <div className="flex-1">
          <Technology technologies={technologies} handleAddToStack={handleAddToStack} added={added} />
        </div>
        <div className="w-[320px] shrink-0 rounded-[20px] border border-[#e8edf3] bg-white p-5">
          ...
          <div className="mt-5 flex flex-col gap-3">
            {addToStacks.map((stack) => (
              <div key={stack.name} className="flex items-center gap-3 rounded-xl
               border border-[#e8edf3] bg-[#f8fafc] px-3 py-2.5">
                <img src={stack.icon} alt={stack.name} className="h-9 w-9 object-contain"/>
                <div className="flex-1">
                ...
                <button onClick={() => handleRemove(stack)}
          ...
          {addToStacks.length > 0 && (
            <button onClick={handleRemoveAll} className="mt-5 h-11 w-full rounded-xl border border-[#ff4d4f] bg-white
             text-sm font-medium text-[#ff4d4f] transition hover:bg-[#fff1f0]">
              Remove All
            </button>
          )}
        </div>
